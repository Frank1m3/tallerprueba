...
class OrdenDeComprasDao:
    ...
    def obtener_ordenes(self):
        ...
        try:
            ...
            return [{
                ...
                'nro_orden': f[1],
                'fecha_emision': f[2].strftime("%Y-%m-%d") if f[2] else None,
                'fun_id': f[3],
                'funcionario': f[4],
                'sucursal': f[5],
                'id_proveedor': f[6],
                'proveedor_nombre': f[7] if f[7] else '',
                'id_pre_compra_cab': f[8],
                'estado': f[9]
            } for f in cur.fetchall()]
        except Exception as e:
    ...
    def obtener_orden_por_id(self, id_orden):
        ...
        try:
            cur.execute("""
                ...
                LEFT JOIN funcionarios f ON f.fun_id = oc.fun_id
                LEFT JOIN sucursal s ON s.id_sucursal = oc.id_sucursal
                LEFT JOIN deposito d ON d.id_deposito = oc.id_deposito
                LEFT JOIN proveedor prov ON prov.id_proveedor = oc.id_proveedor
            ...
            orden = {
                ...
                'nro_orden': fila[1],
                'fecha_emision': fila[2].strftime("%Y-%m-%d") if fila[2] else None,
                'fun_id': fila[3],
                'funcionario': fila[4],
                'id_sucursal': fila[5],
                'sucursal': fila[6],
                ...
                'deposito': fila[8],
                'id_proveedor': fila[9],
                'proveedor_nombre': fila[10] if fila[10] else '',
                'id_pre_compra_cab': fila[11],
                'estado': fila[12],
                'detalle': []
            }
    ...
    def agregar(self, dto: OrdenDeComprasDto) -> int:
        ...
        (nro_orden, id_pre_compra_cab, id_sucursal, id_deposito, fun_id, fecha_emision, id_proveedor, estado)
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT COALESCE(MAX(CAST(NULLIF(regexp_replace(nro_orden, '\\D', '', 'g'), '') AS INTEGER)), 0) + 1 FROM orden_compra_cab")
            seq = cur.fetchone()[0]
            nro_orden = f'OC-{seq:06d}'
            ...
            if dto.id_pre_compra_cab:
                cur.execute("""
                    UPDATE presupuesto_compra_cab
                    SET estado = 'OC_GENERADA'
                    WHERE id_pre_compra_cab = %s
                """, (dto.id_pre_compra_cab,))
